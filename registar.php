...
<?php
    if(auth()){
        ?>
        <p>Já estás com o Login Feito!</p>
        <a href="logout.php">Sair da conta</a>
        <?php
    }else if(isset($_POST['name'],$_POST['email'], $_POST['password']) && $_POST['name'] != '' && $_POST['email'] != '' && $_POST['password'] != ''){

        $nome = htmlentities($_POST['name']);
        $email = htmlentities($_POST['email']);
        $password = htmlentities($_POST['password']);

        if(!verificaEmail($conn, $_POST['email']))
        {
            registar($conn, [
                'nome' => $nome,
                'email' => $email,
            ...
            header('Location:registar.php?fail');
        }
        ...
        {
            ?>
            <p style="color:red">Esse email já está registado!</p>
            <?php
        }elseif(isset($_POST['name'], $_POST['email'], $_POST['password']) && $_POST['name'] === '' && $_POST['email'] === '' && $_POST['password'] === '')
        {
          ?>
            <p style="color:red">Todos os Campos são obrigatorios!</p>
            <?php
        }
        ?>
        <div class="container">
          <div class="row">
            <div class="login">
              <form action="" method="post">
                ...
                <div>
                  <button type="submit" class="btn btn-light">Registar</button>
                </div>
              </form>
            </div>
          </div>
